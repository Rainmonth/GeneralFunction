...
import com.rainmonth.adapter.entity.IExpandable;
...
public abstract class BaseMultiItemQuickAdapter<T extends MultiItemEntity, K extends BaseViewHolder> extends BaseQuickAdapter<T, K> {
    ...
    public int getParentPositionInAll(int position) {
        ...
        if (isExpandable(multiItemEntity)) {
            IExpandable IExpandable = (com.rainmonth.adapter.entity.IExpandable) multiItemEntity;
            for (int i = position - 1; i >= 0; i--) {
                MultiItemEntity entity = data.get(i);
                if (isExpandable(entity) && IExpandable.getLevel() > ((com.rainmonth.adapter.entity.IExpandable) entity).getLevel()) {
                    return i;
                }
            }
        } else {
